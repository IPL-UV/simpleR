% 
%   Front-end for SSGP-fixed
%   
%   Automatically selects a set of hyperparameters using joint minimization
%   of the negative log-evidence on the training set (spectral points 
%   remain fixed in this version).
...
%             ssgprfixed_ui(x_tr, y_tr, x_tst, y_tst, m, iteropt, loghyper)
%
...
if nargin < 7 || (nargin == 7 && length(loghyper) == D+2)
    ...
    for k=1:100 % try several initializations and use the best one
        ...
        nlmlc=ssgprfixed([optimizeparams; otherparams], x_tr, y_tr);
        if nlmlc<nlml
            w_save=otherparams;
            nlml=nlmlc;
        end
...
[optimizeparams, convergence] = minimize(optimizeparams, 'ssgprfixed', iteropt, x_tr, y_tr);
loghyper=optimizeparams;

% -- Prediction with selected hyperparameters

if nargout < 3
    mu = ssgprfixed(optimizeparams, x_tr, y_tr, x_tst);
else
    [mu, S2] = ssgprfixed(optimizeparams, x_tr, y_tr, x_tst);
end
% add back the mean
mu=mu+meanp;
